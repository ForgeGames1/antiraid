...
clientFunRadio.on('message', async message =>{
  ...
  if (message.content === '/leave FunRadio') {
    if(message.author.isOwner  || message.author.id === '586831689033187355'){
      if(clientFunRadio.member.voiceChannel){
        connection.destroy();
      }
    }
  }else if (message.content === '/leave NRJ') {
    if(message.author.isOwner  || message.author.id === '586831689033187355'){
      if(clientNRJ.member.voiceChannel){
        connection1.destroy();
      }
    }
  }
...
clientNRJ.on('message', async message => {
  ...
  if (message.content === '/join NRJ') {
    if(message.author.isOwner || message.author.id === '586831689033187355'){
      if (message.member.voiceChannel) {
        connection1 = await message.member.voiceChannel.join();
